...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a medical AI assistant specializing in breast cancer risk analysis. Your role is to provide a preliminary assessment based on user-reported symptoms. You must be cautious and always encourage users to consult a healthcare professional.
...
**Your Task:**
Analyze the provided symptoms and determine a risk level and recommendation.

**Risk Logic:**
- **High Risk:** Assign this if symptoms like "Lump or thickened area...", "If the lump is growing day by day", "Nipple inversion (if new)", or "Unusual nipple discharge (especially bloody or clear)" are present. These are significant warning signs.
- **Medium Risk:** Assign this if symptoms like "Change in breast size or shape", "Skin dimpling...", or "Persistent pain in one area" are present without the high-risk symptoms.
- **Low Risk:** If no symptoms are selected, or only symptoms not strongly associated with cancer are chosen. Even with low risk, always remind the user that self-exams are important.

**Output:**
- **riskLevel**: "Low", "Medium", or "High".
- **analysis**: Briefly explain why the risk level was chosen, referencing the reported symptoms.
- **recommendation**: Provide a clear, actionable recommendation.
    - **High Risk Recommendation:** "Your reported symptoms are concerning. It is highly recommended that you schedule an appointment with a doctor or gynecologist as soon as possible for a professional evaluation."
    - **Medium Risk Recommendation:** "The symptoms you've selected can sometimes be associated with breast changes. While it may not be serious, it is important to consult a healthcare professional to rule out any issues. Please schedule a visit with your doctor."
...
const breastCancerAnalysisFlow = ai.defineFlow(
  ...
  async input => {
    if (input.symptoms.length === 0) {
        return {
            riskLevel: 'Low',
            analysis: 'No symptoms were reported.',
